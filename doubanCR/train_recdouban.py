...
from utils.GraphMaker1 import GraphMaker
from model.trainer import CDSRTrainer
from utils.loaderDouban import *
import json
import codecs
import pdb
...
# dataset part
parser.add_argument('--data_dir', type=str, default='Douban', help='Food-Kitchen , Movie-Book, Entertainment-Education ,Douban')
...
parser.add_argument('--GNN', type=int, default=2, help='GNN depth.')
...
train_batch = DataLoader1(opt['data_dir'], opt['batch_size'], opt, evaluation = -1)
valid_batch = DataLoader1(opt['data_dir'], opt["batch_size"], opt, evaluation = 2)
test_batch = DataLoader1(opt['data_dir'], opt["batch_size"], opt, evaluation = 1)
         ...
         ##   if pred <= 5:
          ##      NDCG_5 += 1 / np.log2(pred + 1)
            ...
            if valid_entity % 100 == 0:
                print('.', end='')
        return MRR/valid_entity, NDCG_10 / valid_entity, NDCG_20 / valid_entity, HR_1 / valid_entity, HR_10 / valid_entity, HR_20 / valid_entity
